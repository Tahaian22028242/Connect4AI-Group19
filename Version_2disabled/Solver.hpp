...
class Solver {
 private:
  static constexpr int TABLE_SIZE = 26; // store 2^TABLE_SIZE elements in the transpositiontbale
   ...
   */
  int negamax(const Position &P, int alpha, int beta, int depthLeft);

 public:
  static const int INVALID_MOVE = -1000;
  
  // Returns the score of a position
  int solve(const Position &P, bool weak = false, int maxDepth = 99);
